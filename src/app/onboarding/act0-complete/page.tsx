...
import { ACT_0_PHASE_IDS, ACT_1_PHASE_IDS_V2, ALL_PHASE_IDS_V2 } from '@/constants/onboarding';
...
export default function Act0CompletePage() {
  ...
  const {
    ...
    act0GapResult, setCurrentAct, skippedPhaseIds,
  } = useOnboardingStore();
  ...
  const gapDomains = act0GapResult?.coverage?.gapDomains ?? [];
  // Derive strong domains: those with coverage >= 30% that aren't in gapDomains
  const strongDomains = (act0GapResult?.coverage?.domains ?? [])
  ...
  const handleContinue = () => {
    ...
    for (let i = 0; i < ACT_1_PHASE_IDS_V2.length; i++) {
      const phaseId = ACT_1_PHASE_IDS_V2[i];
    ...
    router.push(`/onboarding/phase/${ACT_1_PHASE_IDS_V2[0]}`);
  };

  // Progress ring (Act 0 = ~33% of total)
  const progress = ACT_0_PHASE_IDS.length / ALL_PHASE_IDS_V2.length;
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center px-6 bg-[var(--t-cream)]">
      <div className="max-w-lg w-full flex flex-col items-center">
        ...
        `}>
          {ACT_1_PHASE_IDS_V2.length - skippedPhaseIds.filter((id) =>
            (ACT_1_PHASE_IDS_V2 as readonly string[]).includes(id)
