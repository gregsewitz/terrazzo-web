...
const DEMO_POOL: ImportedPlace[] = [
  ...
  },
  {
    ...
    tasteNote: 'Innovative satoyama cuisine — nature meets fine dining',
  ...
  },
  {
    ...
    tasteNote: 'Bohemian vintage quarter — indie shops & theatres',
    google: { rating: 4.3, reviewCount: 5621, category: 'Neighborhood' },
    terrazzoInsight: { why: 'Your Character axis will love the vintage stores, tiny theatres, and independent coffee scene.', caveat: 'No single anchor attraction — best for aimless wandering. Half-day minimum.' },
    status: 'available',
  },
  {
    id: '5',
  ...
  },
  {
    ...
    tasteNote: 'Traditional hot spring retreat — volcanic mineral waters',
    google: { rating: 4.6, reviewCount: 1543, category: 'Hot Spring' },
    terrazzoInsight: { why: 'Your Wellness axis peaks here — volcanic mineral waters with mountain views.', caveat: 'Full day commitment (90 min each way from Tokyo). Tattoo policies vary.' },
    status: 'available',
  },
  {
    id: '10',
    ...
    tasteNote: 'Street food paradise — the original fish market district',
...
function createDefaultDays(numDays: number): TripDay[] {
  return Array.from({ length: numDays }, (_, i) => ({
    dayNumber: i + 1,
    slots: DEFAULT_TIME_SLOTS.map(s => ({ ...s, place: undefined })),
  }));
}
...
const DEMO_TRIP: Trip = {
  ...
  endDate: '2026-04-14',
  days: createDefaultDays(5),
  pool: DEMO_POOL,
};

// Place the Aman into day 1 morning
DEMO_TRIP.days[0].slots[0].place = DEMO_POOL.find(p => p.id === '5');
