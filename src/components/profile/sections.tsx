...
import { SectionLabel } from './discover-sections/SectionLabel';
import {
  FadeInSection,
  StaggerContainer,
  StaggerItem,
...
// ═══════════════════════════════════════════
export function IdentitySection({ profile, signalCount, numericProfile }: { profile: ProfileShape; signalCount: number; numericProfile: NumericProfile }) {
  return (
    <div style={{ background: 'var(--t-navy)' }}>
      {/* Hero content */}
...
// ═══════════════════════════════════════════
export function TasteDNASection({ profile, mosaicAxes }: { profile: ProfileShape; mosaicAxes: Record<string, number> }) {
  const axes = profile.radarData || [];
  return (
    <div className="py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <div className="px-5">
      <FadeInSection>
        <SectionLabel color={COLOR.coral}>Taste DNA</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
      ...
      <div className="flex flex-col gap-5">
        {axes.map(({ axis, value }, i) => {
          const color = AXIS_COLORS[axis] || 'var(--t-coral)';
...
// ═══════════════════════════════════════════
export function DesignLanguageSection({ profile, gp, mosaicAxes }: { profile: ProfileShape; gp: GeneratedTasteProfile | null; mosaicAxes: Record<string, number> }) {
  ...
  return (
    <div className="py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <div className="px-5">
      <FadeInSection>
        <SectionLabel color={COLOR.teal}>Your Design Eye</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
          How You See Spaces
        </h3>
        {insight?.headline && (
          <p className="text-[16px] mb-2 italic leading-snug" style={{ fontFamily: FONT.serif, color: COLOR.navy }}>
            ...
            <FadeInSection key={a.axis} delay={i * 0.1}>
              <div className="px-4 py-3 rounded-2xl" style={{ background: 'white', boxShadow: '0 1px 3px rgba(0,42,85,0.08)' }}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[12px] uppercase" style={{ color: COLOR.navy, fontFamily: FONT.mono, opacity: 0.6 }}>{a.label[0]}</span>
                  <span className="text-[12px] uppercase" style={{ color: COLOR.navy, fontFamily: FONT.mono, opacity: 0.6 }}>{a.label[1]}</span>
                </div>
                <AnimatedSpectrum percentage={pct} delay={i * 0.12} />
                <p className="text-[13px] mt-2.5 leading-relaxed" style={{ color: COLOR.navy, fontFamily: FONT.sans }}>
                  {a.note}
                </p>
              </div>
            </FadeInSection>
...
// ═══════════════════════════════════════════
// CONTRADICTIONS — Deep dive with editorial split layout
// ═══════════════════════════════════════════
export function ContradictionsSection({ profile }: { profile: ProfileShape }) {
  return (
    <div className="relative overflow-hidden px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.coral}>Your Contradictions</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
      ...
      <StaggerContainer className="flex flex-col gap-4" staggerDelay={0.12}>
        {profile.contradictions.map((c, i) => (
          <StaggerItem key={i}>
            <div className="rounded-2xl overflow-hidden" style={{ background: 'white', boxShadow: '0 2px 8px rgba(0,42,85,0.08)' }}>
              <div className="flex">
                {/* You Said column */}
...
// ═══════════════════════════════════════════
export function PerfectDaySection({ gp }: { gp: GeneratedTasteProfile | null }) {
  const day = gp?.perfectDay;
  if (!day) return null;

  const segments = [
    { time: 'Morning', icon: 'morning' as const, text: day.morning, color: 'var(--t-coral)' },
    { time: 'Afternoon', icon: 'afternoon' as const, text: day.afternoon, color: 'var(--t-ochre)' },
    { time: 'Evening', icon: 'evening' as const, text: day.evening, color: 'var(--t-teal)' },
  ];

  return (
    <div className="px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.coral}>Your Perfect Day</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
              ...
              <div className="flex flex-col items-center" style={{ width: 28 }}>
                {isMobileSafari() ? (
                  <div
                    ...
                    style={{ background: seg.color, boxShadow: `0 2px 8px ${seg.color}40` }}
                  >
                    <PerriandIcon name={seg.icon} size={13} color="white" />
                  </div>
                ) : (
                  <motion.div
                    className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0"
...
// ═══════════════════════════════════════════
export function ContextShiftsSection({ profile, gp }: { profile: ProfileShape; gp: GeneratedTasteProfile | null }) {
  const shifts = gp?.howYouShift;
  return (
    <div className="px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.teal}>How Your Taste Shifts</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
      ...
      {shifts && shifts.length > 0 && (
        <StaggerContainer className="flex flex-col gap-3 mb-6" staggerDelay={0.1}>
          {shifts.map((s, i) => (
            <StaggerItem key={i}>
...
// ═══════════════════════════════════════════
export function TasteNeighborsSection({ gp }: { gp: GeneratedTasteProfile | null }) {
  ...
  return (
    <div className="px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.navy}>Taste Neighbors</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
          Archetypes Like You
        </h3>
        <p className="text-[14px] mb-6" style={{ color: COLOR.navy, fontFamily: FONT.sans }}>
...
export function DimensionsSection({ profile }: { profile: ProfileShape }) {
  ...
  return (
    <div className="px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.navy}>Taste Dimensions</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
      ...
      <StaggerContainer className="flex flex-col gap-3" staggerDelay={0.08}>
        {domains.map(domain => {
          const color = DIMENSION_COLORS[domain] || 'var(--t-coral)';
...
// ═══════════════════════════════════════════
export function VocabularySection({ profile }: { profile: ProfileShape }) {
  ...
  for (const [category, terms] of Object.entries(profile.microTasteSignals)) {
    if (category === 'Rejection') {
      ...
      const color = DIMENSION_COLORS[category] || 'var(--t-coral)';
  ...
  return (
    <div className="px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.ochre}>Taste Vocabulary</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
...
// ═══════════════════════════════════════════
export function MatchesSection({ profile }: { profile: ProfileShape }) {
  return (
    <div className="px-5 py-8 pb-10" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.teal}>Top Matches</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
          Places Made for You
        </h3>
        <p className="text-[14px] mb-6" style={{ color: COLOR.navy, fontFamily: FONT.sans }}>
...
export function TasteRadarSection({ profile }: { profile: ProfileShape }) {
  const axes = profile.radarData || [];
        ...
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          <defs>
            <radialGradient id="radarAnimFill" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor="var(--t-coral)" stopOpacity={0.15} />
              <stop offset="100%" stopColor="var(--t-coral)" stopOpacity={0.04} />
            </radialGradient>
          </defs>

          {/* Grid polygons */}
          {gridPolygons.map((points, i) => (
            <polygon key={`g-${i}`} points={points} fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth={0.5} />
          ))}

          {/* Axis lines */}
          {axes.map((d, i) => {
            const outer = getPoint(i, maxRadius);
            const color = AXIS_COLORS[d.axis] || 'var(--t-coral)';
            return <line key={`a-${i}`} x1={cx} y1={cy} x2={outer.x} y2={outer.y} stroke={color} strokeOpacity={0.3} strokeWidth={0.8} />;
          })}

          {/* Animated data polygon */}
          {isMobileSafari() ? (
            <polygon points={dataPolygon} fill="url(#radarAnimFill)" stroke="var(--t-coral)" strokeWidth={2} strokeOpacity={0.8} />
          ) : (
            <motion.polygon
          ...
          {/* Data point dots */}
          {dataPoints.map((p, i) => {
            const color = AXIS_COLORS[axes[i].axis] || 'var(--t-coral)';
            return isMobileSafari() ? (
              <g key={`d-${i}`}>
                <circle cx={p.x} cy={p.y} r={4} fill={color} />
                <circle cx={p.x} cy={p.y} r={1.5} fill="var(--t-navy)" />
              </g>
            ) : (
          ...
          {axes.map((d, i) => {
            ...
            return (
              <text key={`l-${i}`} x={lp.x} y={lp.y} textAnchor="middle" dominantBaseline="middle" fill="var(--t-peach)" fontSize={11} fontFamily={FONT.mono} opacity={0.8} fontWeight={600}>
                {d.axis}
              </text>
            );
          })}
          ...
          {axes.map((d, i) => {
            ...
            const color = AXIS_COLORS[d.axis] || 'var(--t-coral)';
            return (
              <text key={`v-${i}`} x={vp.x} y={vp.y} textAnchor="middle" dominantBaseline="middle" fill={color} fontSize={10} fontFamily={FONT.display} fontWeight={700} opacity={0.95}>
...
function AnimatedStatCard({ value, label, suffix, delay }: { value: number; label: string; suffix?: string; delay: number }) {
  ...
  return (
    <FadeInSection delay={delay} direction="up" distance={20}>
      <div className="text-center p-4 rounded-2xl" style={{ background: 'white', boxShadow: '0 2px 8px rgba(0,42,85,0.1)' }}>
  ...
  return (
    <FadeInSection delay={delay} direction="left" distance={24}>
      <div ref={ref} className="flex items-center gap-3 mb-4">
        <span className="text-[14px] font-bold w-32 shrink-0" style={{ color: COLOR.navy, fontFamily: FONT.display }}>{name}</span>
        ...
        <span className="text-[13px] w-8 text-right font-bold" style={{ color: COLOR.navy, fontFamily: FONT.mono }}>{pct}%</span>
      </div>
    </FadeInSection>
  );
}
...
export function TravelStatsSection() {
  return (
    <div className="px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.coral}>Your Travel Stats</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
      ...
      <div className="p-5 rounded-2xl" style={{ background: 'var(--t-navy)' }}>
        {CONTINENTS.map((c, i) => (
...
const TRIPS: Array<{ title: string; dates: string; places: number; upcoming: boolean; color: string; icon: import('@/types').PerriandIconName }> = [
  ...
  { title: 'Puglia Slow Trip', dates: 'Sep 1\u201310, 2024', places: 11, upcoming: false, color: '#413800', icon: 'food' },
];

export function TravelTimelineSection() {
  return (
    <div className="px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.navy}>Travel Timeline</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
          Your Journey So Far
        </h3>
        <p className="text-[14px] mb-6" style={{ color: COLOR.navy, fontFamily: FONT.sans }}>
      ...
      <div className="relative">
        ...
        {!isMobileSafari() ? (
          ...
          <div
            className="absolute left-[15px] top-0 w-[2px] h-full"
            style={{ background: 'var(--t-navy)' }}
          />
        )}
                  ...
                  {isMobileSafari() ? (
                    <div
                      ...
                      style={{
                        ...
                        border: trip.upcoming ? 'none' : `2.5px solid ${trip.color}`,
                        boxShadow: trip.upcoming ? `0 4px 12px ${trip.color}40` : 'none',
                      }}
                    >
                      <PerriandIcon name={trip.icon} size={15} color={trip.upcoming ? 'white' : trip.color} />
                    </div>
                  ) : (
                    <motion.div
                      className="w-8 h-8 rounded-full flex items-center justify-center shrink-0"
...
export function TasteEvolutionSection({ profile }: { profile: ProfileShape }) {
  ...
  return (
    <div className="px-5 py-8" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.coral}>Taste Evolution</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
          How You&apos;ve Grown
        </h3>
        <p className="text-[14px] mb-6" style={{ color: COLOR.navy, fontFamily: FONT.sans }}>
      ...
      <div className="relative">
        ...
        {!isMobileSafari() ? (
          <motion.div
            ...
            style={{ background: `linear-gradient(to bottom, ${COLOR.coral}, ${COLOR.navy})` }}
            initial={{ height: 0 }}
          ...
          <div className="absolute left-[7px] top-2 w-[2px]" style={{ height: 'calc(100% - 16px)', background: `linear-gradient(to bottom, ${COLOR.coral}, ${COLOR.navy})` }} />
        )}
              ...
              <FadeInSection key={phase.period} delay={i * 0.15} direction="left" distance={24}>
                <div className="flex gap-4">
                  ...
                  <div className="shrink-0 mt-1">
                    {isMobileSafari() ? (
                      <div className="w-4 h-4 rounded-full border-2" style={{ borderColor: isCurrent ? COLOR.coral : phase.color, background: isCurrent ? COLOR.coral : 'white' }} />
                    ) : (
                      <motion.div
                        className="w-4 h-4 rounded-full border-2"
                        style={{ borderColor: isCurrent ? COLOR.coral : phase.color, background: isCurrent ? COLOR.coral : 'white' }}
                        initial={{ scale: 0 }}
                        whileInView={{ scale: 1 }}
...
export function BucketListSection() {
  ...
  return (
    <div className="px-5 py-8 pb-10" style={{ borderBottom: `1px solid ${INK['06']}` }}>
      <FadeInSection>
        <SectionLabel color={COLOR.coral}>The Bucket List</SectionLabel>
        <h3 className="text-[22px] font-bold mb-2" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
          Dream Destinations
        </h3>
        <p className="text-[14px] mb-6" style={{ color: COLOR.navy, fontFamily: FONT.sans }}>
      ...
      <FadeInSection delay={0.15}>
        <div className="flex items-center gap-4 p-5 rounded-2xl mb-6" style={{ background: 'white', boxShadow: '0 2px 8px rgba(0,42,85,0.08)' }}>
          <ProgressRing progress={totalProgress} color={COLOR.coral} size={56} delay={0.2} />
          <div className="flex-1">
            <div className="text-[17px] font-bold mb-0.5" style={{ color: COLOR.navy, fontFamily: FONT.display }}>
              {completedCount > 0 ? `${completedCount} nearly ready` : 'Building your list'}
            </div>
            <div className="text-[13px] font-medium" style={{ color: COLOR.navy, fontFamily: FONT.sans }}>
      ...
      <StaggerContainer className="flex flex-col gap-4" staggerDelay={0.1}>
        {BUCKET_LIST.map((item, i) => (
          <StaggerItem key={item.destination}>
            <div className="flex items-center gap-4 p-4 rounded-2xl" style={{ background: 'var(--t-cream)', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', borderLeft: `3px solid ${item.color}` }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[13px] leading-relaxed mb-2" style={{ color: COLOR.navy, fontFamily: FONT.sans }}>{item.reason}</p>
