  ...
  return (
    <>
      <div className="mt-1 mb-1">
        <h2 className="text-xl italic" style={{ fontFamily: FONT.serif, color: TEXT.primary }}>
          Found {importResults.length} places
        </h2>
        <p className="text-[10px] mt-0.5" style={{ color: TEXT.secondary }}>
      ...
      {sourceName && (
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg mb-4 mt-2" style={{ background: 'rgba(199,82,51,0.06)' }}>
          ...
          <span className="text-[10px] font-semibold" style={{ color: TEXT.accent }}>
            {sourceName}
          </span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onDeselectAll} className="text-[10px] font-semibold bg-transparent border-none cursor-pointer" style={{ color: TEXT.secondary }}>
            Clear
          </button>
        </div>
      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {item.tasteNote && (
                          <div className="text-[10px] truncate" style={{ color: TEXT.secondary }}>
      ...
      <button
        ...
        style={{
          background: selectedIds.size > 0 ? TEXT.primary : 'rgba(0,0,0,0.10)',
          color: selectedIds.size > 0 ? 'white' : TEXT.secondary,
        }}
      >
        Save {selectedIds.size} places
        <PerriandIcon name="terrazzo" size={16} color={selectedIds.size > 0 ? 'white' : INK['90']} />
      </button>
      <p className="text-center text-[10px] mt-1.5 mb-2" style={{ color: TEXT.secondary }}>
        Deselect any you don&apos;t want
        {detectedDestination ? ` · All go to ${detectedDestination}` : ''}
      </p>

      <button onClick={onImportMore} className="w-full mt-1 py-2 bg-transparent border-none cursor-pointer text-[11px]" style={{ color: TEXT.secondary }}>
        ← Import more
      </button>
    </>
  );
