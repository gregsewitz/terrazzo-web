  ...
  if (isDesktop) {
    return (
      <>
        ...
        >
          {/* Photo header — taller on desktop */}
          <div
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 flex items-center justify-center border-none cursor-pointer"
              style={{
                width: 36,
                height: 36,
                borderRadius: '50%',
              ...
              onMouseEnter={e => (e.currentTarget.style.background = 'rgba(28,26,23,0.6)')}
              onMouseLeave={e => (e.currentTarget.style.background = 'rgba(28,26,23,0.45)')}
            >
              ✕
            </button>
          </div>

          {/* Scrollable content — more padding on desktop */}
          <div className="flex-1 overflow-y-auto px-7 pb-8">
            {/* Save CTA for previews */}
            {isPreview && onSave && (
              <button
                onClick={onSave}
                className="w-full mb-3 py-3 rounded-xl border-none cursor-pointer text-[14px] font-semibold transition-all btn-hover flex items-center justify-center gap-2 mt-5"
                style={{
            ...
            <div className="flex items-start gap-2 mt-5">
              <h2
                className="text-[24px] italic leading-tight flex-1 min-w-0"
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0 mt-0.5">
                {existingRating && ratingReaction ? (
                  <button
                    onClick={onRate}
                    className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg cursor-pointer transition-all btn-hover border-none"
                    style={{
                      background: `${ratingReaction.color}10`,
                      border: `1.5px solid ${ratingReaction.color}25`,
                    }}
                  >
                    <PerriandIcon name={ratingReaction.icon} size={14} color={ratingReaction.color} />
                  ...
                  >
                    <PerriandIcon name="star" size={13} color={INK['80']} />
                ...
                {!isPreview && onShortlistTap && (
                  ...
                  >
                    <PerriandIcon name="bookmark" size={12} color={isShortlisted ? 'var(--t-verde)' : INK['70']} />
            ...
            <div className="flex items-center gap-1.5 mt-2 mb-4 flex-wrap">
              <span className="text-[12px]" style={{ color: INK['70'] }}>
            ...
            {item.enrichment?.description && (
              <p className="text-[13px] leading-relaxed mb-4" style={{ color: INK['85'] }}>
                {item.enrichment.description}
              </p>
            )}
            ...
            {item.enrichment?.closedDays && item.enrichment.closedDays.length > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg mb-4" style={{ background: 'rgba(232,104,48,0.12)', border: '1px solid rgba(232,104,48,0.25)' }}>
                <span>⚠️</span>
                <span className="text-[12px] font-medium" style={{ color: '#a04018' }}>Closed {item.enrichment.closedDays.join(', ')}s</span>
              </div>
            )}
            ...
            {item.whatToOrder && item.whatToOrder.length > 0 && (
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {item.whatToOrder.map((tag, i) => (<div key={i} className="px-2.5 py-1 rounded-lg text-[12px]" style={{ background: 'var(--t-linen)', color: 'var(--t-ink)' }}>{tag}</div>))}
                </div>
              </div>
            )}
            ...
            {item.tips && item.tips.length > 0 && (
              <div className="mb-4">
                ...
                <div className="rounded-xl px-3 py-2.5" style={{ background: 'var(--t-linen)' }}>
                  {item.tips.map((tip, i) => (<div key={i} className="text-[12px] leading-relaxed" style={{ color: 'var(--t-ink)' }}>{tip}</div>))}
                </div>
              </div>
            )}
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                  {siblingPlaces.slice(0, 5).map(sibling => (
                    <div key={sibling.id} className="min-w-[140px] rounded-xl p-2.5 flex-shrink-0 card-hover" style={{ background: 'white', border: '1px solid var(--t-linen)' }}>
                      <div className="text-[12px] font-semibold" style={{ color: 'var(--t-ink)' }}>{sibling.name}</div>
                      <div className="text-[10px]" style={{ color: INK['95'] }}>{sibling.type.charAt(0).toUpperCase() + sibling.type.slice(1)} · {sibling.location.split(',')[0]}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {item.tasteNote && (
              <div className="mb-4" style={{ background: sourceStyle ? `${sourceStyle.color}14` : 'rgba(199,82,51,0.08)', borderLeft: `3px solid ${sourceStyle?.color || '#c75233'}`, padding: '12px 14px', borderRadius: '0 12px 12px 0' }}>
                ...
                <p className="text-[13px] leading-relaxed" style={{ color: 'var(--t-ink)' }}>{item.tasteNote}</p>
              </div>
            )}
            ...
            {item.friendAttribution && (
              <div className="mb-4" style={{ background: 'rgba(42,122,86,0.08)', borderLeft: '3px solid var(--t-verde)', padding: '12px 14px', borderRadius: '0 12px 12px 0' }}>
                ...
                {item.friendAttribution.note && <p className="text-[13px] italic leading-relaxed" style={{ color: 'var(--t-ink)' }}>&ldquo;{item.friendAttribution.note}&rdquo;</p>}
              </div>
            )}

            {/* Your notes */}
            {existingRating && ratingReaction && (existingRating.tags?.length || existingRating.personalNote || existingRating.contextTags?.length) && (
              <div className="mb-4 px-3 py-2.5 rounded-xl cursor-pointer" style={{ background: `${ratingReaction.color}06`, border: `1px solid ${ratingReaction.color}15` }} onClick={onEditRating || onRate}>
                ...
                {existingRating.personalNote && <p className="text-[12px] italic mt-1.5" style={{ color: INK['90'] }}>&ldquo;{existingRating.personalNote}&rdquo;</p>}
              </div>
            )}
            ...
            {item.terrazzoInsight && (
              <div className="flex flex-col gap-3 mb-4">
                <div className="p-3 rounded-xl" style={{ background: 'rgba(42,122,86,0.10)', border: '1px solid rgba(42,122,86,0.20)' }}>
                  ...
                  <p className="text-[12px] leading-relaxed" style={{ color: 'var(--t-ink)' }}>{item.terrazzoInsight.why}</p>
                </div>
                {item.terrazzoInsight.caveat && (
                  <div className="p-3 rounded-xl" style={{ background: 'rgba(160,108,40,0.10)', border: '1px solid rgba(160,108,40,0.20)' }}>
                    ...
                    <p className="text-[12px] leading-relaxed" style={{ color: 'var(--t-ink)' }}>{item.terrazzoInsight.caveat}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {/* Match score */}
            <div className="flex items-center gap-3 p-3 rounded-xl mb-4" style={{ background: 'rgba(200,146,58,0.10)', border: '1px solid rgba(200,146,58,0.20)', cursor: onViewBriefing ? 'pointer' : 'default' }} onClick={onViewBriefing}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center text-[15px] font-bold flex-shrink-0" style={{ background: 'linear-gradient(135deg, rgba(200,146,58,0.2), rgba(200,146,58,0.1))', color: '#8a6a2a', fontFamily: FONT.mono }}>{item.matchScore}%</div>
              <div className="flex-1 min-w-0">
                <div className="text-[13px] font-semibold" style={{ color: 'var(--t-ink)' }}>Taste match</div>
                <div className="text-[11px]" style={{ color: INK['95'] }}>Based on your profile preferences</div>
                ...
                {onViewBriefing && googlePlaceId && (<button className="text-[11px] mt-1 block border-none bg-transparent p-0 cursor-pointer" style={{ color: '#7a5a20', fontFamily: FONT.mono }} onClick={(e) => { e.stopPropagation(); onViewBriefing(); }}>View full briefing →</button>)}
              </div>
            </div>
            ...
            {onRate && (
              <button
                onClick={onRate}
                className="w-full mt-6 py-3.5 rounded-xl border-none cursor-pointer text-[14px] font-semibold btn-hover"
                style={{
